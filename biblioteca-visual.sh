#!/usr/bin/env bash
# Biblioteca mas atractiva: tarjetas visuales de canales (estilo YouTube) y podcasts.
# Uso:  bash biblioteca-visual.sh   (desde la raiz del repo qwen-ufo)
set -e
if [ ! -f package.json ] || [ ! -d .git ]; then
  echo "ERROR: corre este script desde la raiz del repo qwen-ufo."; exit 1
fi
echo ">> Sincronizando main y creando rama feat/biblioteca-visual..."
git checkout main
git pull origin main
git checkout -B feat/biblioteca-visual origin/main
echo ">> Escribiendo archivo..."
mkdir -p src/pages
cat > src/pages/Biblioteca.tsx <<'__AURORA_EOF__'
...
__AURORA_EOF__
echo ">> Build de verificacion..."
npm run build
echo ">> Commit..."
git add src/pages/Biblioteca.tsx
git commit -m "Biblioteca mas atractiva: tarjetas visuales de canales y podcasts"
echo ""
echo "=========================================================="
echo " LISTO. Subilo:"
echo "   git push -u origin feat/biblioteca-visual"
echo " PR: https://github.com/fernicher/qwen-ufo/compare/main...feat/biblioteca-visual"
echo "=========================================================="
