...
import { Search, X, Film, MapPin, Command, ArrowRight } from 'lucide-react';
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed top-[15%] left-1/2 -translate-x-1/2 z-[2001] w-full max-w-2xl bg-aurora-charcoal/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
                ...
                <input 
                  ...
                  placeholder="Buscar películas, casos..." 
              ...
              <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                {results.length === 0 ? (
                  ...
                  <div className="p-2">
                    {results.map((r) => (
                      <button key={r.id} onClick={() => { setIsOpen(false); navigate(r.caseId ? `/mapa?case=${r.caseId}` : `/catalogo?q=${encodeURIComponent(query)}`); }} className="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-white/5 text-left">
